let currentStep = 0;
const TOTAL_STEPS = 13; // added life stage

const STEPS = [
  stepName, stepGoals, stepGender, stepCycleTracking, stepLifeStage,
...
function onboardingBack() {
  if (currentStep > 0) { currentStep--; document.getElementById('app').innerHTML = buildOnboardingScreen(); }
}
...
function saveStepData(step) {
  switch(step) {
    ...
    case 4: USER.lifeStage = getSelectedSingle('lifestage'); break;
    case 5: USER.culturalFood = getSelectedSingle('cultural'); break;
    case 6: USER.cuisines = getSelectedValues('cuisines'); break;
    case 7: USER.dietStyle = getSelectedSingle('diet'); break;
    case 8:
    ...
    case 9:
      ...
      break;
    case 10: USER.conditions = getSelectedValues('conditions').filter(v=>v!=='none'&&v!=='private'); break;
    case 11: break;
    case 12: USER.communityRole = getSelectedSingle('community-role'); break;
  }
}
...
function stepGoals() {
  const opts = [
    ...
    {value:'fitness',emoji:'💪',label:'Fitness performance',hint:'Strength, endurance, or muscle'},
...
function stepGender() {
  const opts = [
    ...
    {value:'nonbinary',emoji:'🧑',label:'Non-binary / gender diverse'},
    {value:'private',emoji:'🔒',label:'Prefer not to say'},
  ];
  return `
    <div class="animate-in">
      <div class="onboarding-question">How do you identify?</div>
      <div class="onboarding-sub">Gender affects hormones, nutrition, and health risks. Private and never shared.</div>
      <div class="option-list">
        ${opts.map(o=>`
...
function stepCycleTracking() {
  ...
  const opts = [
    {value:'yes',emoji:'📅',label:'Yes, I track my cycle',hint:'We\'ll give phase-specific tips'},
    {value:'want',emoji:'🌱',label:'I want to start tracking',hint:'gUide will help you understand your patterns'},
    {value:'no',emoji:'🚫',label:'No, not right now',hint:'You\'ll still get great recommendations'},
  ...
  return `
    <div class="animate-in">
      ...
      <div class="onboarding-sub">Your cycle phase affects energy, appetite, recovery, and mood.</div>
      <div class="option-list">
        ${opts.map(o=>`
...
function stepLifeStage() {
  ...
  return `
    <div class="animate-in">
      <div class="onboarding-question">What life stage are you in?</div>
      <div class="onboarding-sub">Your nutritional needs shift significantly through life. This helps gUide give you age-appropriate tips and meal suggestions.</div>
      <div class="option-list">
        ${opts.map(o=>`
...
function stepCulturalFood() {
  // No hint text — clean titles only
  const opts = [
    {value:'south_asian',emoji:'🍛',label:'South Asian'},
    {value:'east_asian',emoji:'🍜',label:'East & South-East Asian'},
    {value:'middle_eastern',emoji:'🥙',label:'Middle Eastern'},
    {value:'mediterranean',emoji:'🫒',label:'Mediterranean'},
    {value:'indigenous',emoji:'🌿',label:'Indigenous Australian'},
    {value:'pacific',emoji:'🥥',label:'Pacific Islander'},
    {value:'african',emoji:'🌍',label:'African'},
    {value:'latin',emoji:'🌽',label:'Latin American'},
    {value:'none',emoji:'🍽️',label:'No strong food tradition'},
  ];
  return `
    <div class="animate-in">
      <div class="onboarding-question">What's your cultural food background?</div>
      <div class="onboarding-sub">Your traditional foods are part of your health story. gUide suggests meals that feel familiar — not foreign.</div>
      <div class="option-list">
        ${opts.map(o=>`
...
function stepCuisines() {
  const opts = [
    ...
    {value:'african',emoji:'🌍',label:'African'},
    {value:'latin',emoji:'🌽',label:'Latin American'},
    {value:'modern_aus',emoji:'🥗',label:'Modern Australian'},
    {value:'any',emoji:'🌐',label:'All sorts — I eat everything'},
  ];
  return `
    <div class="animate-in">
      <div class="onboarding-question">What cuisines do you actually eat and enjoy?</div>
      <div class="onboarding-sub">Select all that apply — meal suggestions will reflect what you actually cook at home.</div>
      <div class="option-list">
        ${opts.map(o=>`
...
function stepDietStyle() {
  const opts = [
    {value:'omnivore',emoji:'🍖',label:'Omnivore',hint:'Eat meat, dairy, and everything'},
    ...
    {value:'vegan',emoji:'🌱',label:'Vegan',hint:'No animal products'},
    ...
    {value:'other',emoji:'⚙️',label:'Other / it\'s complex',hint:''},
  ];
  return `
    <div class="animate-in">
      <div class="onboarding-question">How would you describe your eating style?</div>
      <div class="onboarding-sub">Meal recommendations will only include things that fit.</div>
      <div class="option-list">
        ${opts.map(o=>`
...
function stepFitnessProtein() {
  ...
  return `
    <div class="animate-in">
      <div class="onboarding-question">Fitness & protein</div>
      <div class="onboarding-sub">Be honest — where you actually are makes recommendations useful.</div>
      <div style="font-size:12px;font-weight:700;color:var(--navy);margin-bottom:8px">Fitness goals <span style="font-size:10px;font-weight:400;color:var(--text-light)">(select all that apply)</span></div>
      ...
      <div style="font-size:12px;font-weight:700;color:var(--navy);margin-bottom:8px">How active are you right now?</div>
      <div class="option-list" style="margin-bottom:16px">
        ${activity.map(o=>`
      ...
      <div class="scale-wrapper">
        ...
        <div class="scale-dots">${[1,2,3,4,5].map(n=>`<button class="scale-dot ${USER.proteinScore===n?'selected':''}" data-group="protein" data-val="${n}" onclick="selectScale(this,'protein')">${n}</button>`).join('')}</div>
...
function stepSleepStressHydration() {
  ...
  return `
    <div class="animate-in">
      <div class="onboarding-question">Sleep, stress & habits</div>
      <div class="onboarding-sub">These hugely impact nutrition and hormones. Honest = better insights.</div>
      <div class="scale-wrapper" style="margin-bottom:12px">
        <div class="scale-label">Sleep quality overall? (1 = terrible, 5 = great)</div>
      ...
      <div class="scale-wrapper" style="margin-bottom:12px">
        <div class="scale-label">Stress level lately? (1 = very low, 5 = very high)</div>
      ...
      <div class="scale-wrapper" style="margin-bottom:16px">
        <div class="scale-label">How well do you hydrate? (1 = rarely, 5 = always)</div>
      ...
      <div style="font-size:12px;font-weight:700;color:var(--navy);margin-bottom:8px">How often do you cook at home?</div>
      <div class="option-list">
        ${cooking.map(o=>`
...
function stepConditions() {
  ...
  return `
    <div class="animate-in">
      <div class="onboarding-question">Any health conditions?</div>
      <div class="onboarding-sub">Private and never shared. Helps ensure recommendations are appropriate for your body.</div>
      <div class="option-list">
        ${opts.map(o=>`
...
function stepConnectApps() {
  ...
  return `
    <div class="animate-in">
      <div class="onboarding-question">Connect your apps</div>
      <div class="onboarding-sub">gUide reads your data — no manual entry. More apps = more personalised. Optional.</div>
      <div class="app-connect-list">
        ${apps.map(a=>`
          <div class="app-connect-item ${USER.connectedApps.includes(a.id)?'connected':''}" id="app-${a.id}" onclick="toggleAppConnect('${a.id}')">
            ...
            <div><div class="app-connect-name">${a.name}</div><div style="font-size:10px;color:var(--text-light);margin-top:1px">${a.desc}</div></div>
...
function toggleAppConnect(appId) {
  const el=document.getElementById(`app-${appId}`); const st=document.getElementById(`status-${appId}`);
  if(USER.connectedApps.includes(appId)){USER.connectedApps=USER.connectedApps.filter(a=>a!==appId);el.classList.remove('connected');st.textContent='Tap to connect';}
  else{USER.connectedApps.push(appId);el.classList.add('connected');st.textContent='✓ Connected';}
}

function stepCommunityRole() {
  const opts = [
    {value:'reader',emoji:'📖',label:'Reader',hint:'Browse tips, no pressure to post'},
    {value:'sharer',emoji:'✍️',label:'Sharer',hint:'Contribute what works for you — earn trust ratings'},
    {value:'both',emoji:'🤝',label:'Both',hint:'Do whatever feels right'},
  ];
  return `
    <div class="animate-in">
      <div class="onboarding-question">How do you want to show up in the community?</div>
      <div class="onboarding-sub">Built on shared experience — not influencers or sponsored content.</div>
      <div class="option-list">
        ${opts.map(o=>`
...
function renderDoneScreen() {
  ...
  if (USER.lifeStage) badges.push(getLifeStageLabel(USER.lifeStage));
  ...
  return `
    <div class="screen onboarding-screen">
      <div class="onboarding-top"><div class="progress-bar-track"><div class="progress-bar-fill" style="width:100%"></div></div></div>
      <div class="done-screen">
        <div class="done-emoji">✨</div>
        <div class="done-title">You're all set, ${USER.name||'friend'}!</div>
        <div class="done-sub">Your gUide is personalised and ready.</div>
